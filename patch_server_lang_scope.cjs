...
// Move the language parsing BEFORE the error responses in the API handler
const parseLangCode = `
...
const regexRemove = /const lang = \(req\.headers\['x-user-lang'\].*?const i18n = t\[lang\] \|\| t\['en'\];/s;
code = code.replace(regexRemove, '');

// Insert at the beginning of the route handler
code = code.replace(
  `  try {
